...
public class Board {
	...
	public void makeMove(Move move) {
		int r;
		boolean moveMade;
		for (r= NUM_ROWS - 1, moveMade= false;  r >= 0 && moveMade == false; r--) {
			if (board[0][move.getColumn()] != null) {
				throw new IllegalArgumentException(
						"Illegal Move: Cannot place disc in full column.");
			}

			if (board[r][move.getColumn()] == null) {
				board[r][move.getColumn()]= move.getPlayer();
				moveMade= true;
			}
		}
	}
	 ...
	 *  Note: If the board has a winner (4 things of the same color in a row), or
